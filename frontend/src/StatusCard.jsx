...
const POLL_MS = 1500000000 // auto-refresh interval
...
async function realFetch(signal) {
  const res = await fetch(`${API_URL}/api/state`, { signal })
  ...
  return (
    ...
    >
      ...
      <Shell serviceName={data?.serviceName}>
        ...
        {!error && data?.latest && (
          <CardBody data={data} refreshing={loading} />
        )}
      </Shell>
    </div>
  )
  ...
  return (
    <>
      <div style={{ padding: '0 18px', marginTop: -1 }}>
        ...
        >
          ...
          >
            ...
            {latest.status}
            {refreshing && (
              <span style={{ color: '#52525b', marginLeft: 2 }}>·</span>
            )}
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: '8px 18px 18px' }}>
        ...
        <Row
          label="Commit"
          value={`${latest.commit} · ${latest.branch}`}
          mono
        />
        <Row label="Updated" value={timeAgo(latest.createdAt)} />
        <Row
          label="Deployed By"
          value={latest.commitAuthor ? `${latest.commitAuthor}` : '—'}
        />
